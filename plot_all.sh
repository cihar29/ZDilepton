...
if [ $# -eq 2 ] ; then
  ...
  for ((i=0;i<${#hists[@]};++i)); do
    ...
    lines=( "dataFileName   ${dir}Muon_${channel}.root"
            "dataName       Data"
            "plotData       true"
            "mcFileNames    ${dir}TTbar_weighted_V2_${channel}.root ${dir}highDY_${channel}.root ${dir}lowDY_${channel}.root ${dir}STschannel_${channel}.root ${dir}STtchannel_${channel}.root ${dir}SaTtchannel_${channel}.root ${dir}STtWchannel_${channel}.root ${dir}SaTtWchannel_${channel}.root ${dir}Wjet_${channel}.root"
