...
string channel, testLeg, tagtrig, probetrig, jet_type, res_era;
vector<string> eras;
...
vector<double> eta_rebin;
vector<double> pT_rebin;
...
int main(int argc, char* argv[]){
  ...
  string probtrig, probelep;
  int pt_bins=500;
  if (testLeg=="muLeg") {
    channel = "em"; probelep = "subMu"; 
    pT_rebin= {25, 55, 60, 80, 120, 200, 300, 400, 800}; 
    eta_rebin = {0, 0.9, 1.2, 2.1, 2.4};
    pt_bins = 800;
  }
  else if (testLeg=="eleLeg"){ 
    channel = "ee"; 
    probelep = "subEle"; 
    pT_rebin= {0, 36 , 50 , 80 ,110, 200};
    eta_rebin = {0, 0.8, 1.4442, 1.5660, 2.0, 2.5};
    pt_bins = 500;
  }
...
"UP")    weight0 *= countTotal / q2UP;
  ...
  int nDirs = 8;
  vector<string> trig_sel = {"BS", "AS"}; // before and after selections
  vector<string> trig_leg = {tagtrig, probetrig}; // tag and probe triggers (change in txt file)
...
/////////////////Trigger Eff Ele leg - Before and after selection - none, Onnb and Onbt/////////////////////////

  for (int j=0; j<int(trig_leg.size()); j++){
    for (int i=0; i<int(trig_sel.size()); i++){
      TString hname = Form("%s_%s_eta_pT_%s",trig_leg[j].data(),probelep.data(),trig_sel[i].data());
      m_Histos2D[hname] = new TH2F(hname,hname,eta_rebin.size()-1,&eta_rebin[0],pT_rebin.size()-1,&pT_rebin[0]);
      hname = Form("%s_%s_pT_%s",trig_leg[j].data(),probelep.data(),trig_sel[i].data());
      m_Histos1D[hname] = new TH1F(hname,hname,pt_bins,0,pt_bins);
      hname = Form("%s_%s_eta_%s",trig_leg[j].data(),probelep.data(),trig_sel[i].data());
      m_Histos1D[hname] = new TH1F(hname,hname,50,-5,5);
      hname = Form("%s_%s_rmin_%s",trig_leg[j].data(),probelep.data(),trig_sel[i].data());
      m_Histos1D[hname] = new TH1F(hname,hname,50,0,5);

      hname = Form("Onnb_%s_%s_eta_pT_%s",trig_leg[j].data(),probelep.data(),trig_sel[i].data());
      m_Histos2D[hname] = new TH2F(hname,hname,eta_rebin.size()-1,&eta_rebin[0],pT_rebin.size()-1,&pT_rebin[0]);
      hname = Form("Onnb_%s_%s_pT_%s",trig_leg[j].data(),probelep.data(),trig_sel[i].data());
      m_Histos1D[hname] = new TH1F(hname,hname,pt_bins,0,pt_bins);
      hname = Form("Onnb_%s_%s_eta_%s",trig_leg[j].data(),probelep.data(),trig_sel[i].data());
      m_Histos1D[hname] = new TH1F(hname,hname,50,-5,5);
      hname = Form("Onnb_%s_%s_rmin_%s",trig_leg[j].data(),probelep.data(),trig_sel[i].data());
      m_Histos1D[hname] = new TH1F(hname,hname,50,0,5);

      hname = Form("Onbt_%s_%s_eta_pT_%s",trig_leg[j].data(),probelep.data(),trig_sel[i].data());
      m_Histos2D[hname] = new TH2F(hname,hname,eta_rebin.size()-1,&eta_rebin[0],pT_rebin.size()-1,&pT_rebin[0]);
      hname = Form("Onbt_%s_%s_pT_%s",trig_leg[j].data(),probelep.data(),trig_sel[i].data());
      m_Histos1D[hname] = new TH1F(hname,hname,pt_bins,0,pt_bins);
      hname = Form("Onbt_%s_%s_eta_%s",trig_leg[j].data(),probelep.data(),trig_sel[i].data());
      m_Histos1D[hname] = new TH1F(hname,hname,50,-5,5);
      hname = Form("Onbt_%s_%s_rmin_%s",trig_leg[j].data(),probelep.data(),trig_sel[i].data());
      m_Histos1D[hname] = new TH1F(hname,hname,50,0,5);
    }
  }
 ...
 /* string triggers[nTriggers] = {
  ...
  for (Long64_t n=0; n<nEntries; n++) {
    ...
    // order of saved triggers are different in 2016 vs 2017, this makes sure we get the right one
    int DoubleEle_trig_id = -1 ; int Ele115_trig_id = -1;
    int Mu50_trig_id = -1; int TkMu50_trig_id= -1;
    for (int i =0; i< int((*trig_passed).size()); i++){
      if ( (*trig_name)[i].find("HLT_DoubleEle33_CaloIdL")!= string::npos  ) {
        DoubleEle_trig_id = i; trg_prefix[DoubleEle_trig_id] = "HLT_DoubleEle33_CaloIdL_"; 
      }
      else if ( (*trig_name)[i].find("HLT_Ele115_CaloIdVT")!= string::npos      ) { 
        Ele115_trig_id = i; trg_prefix[Ele115_trig_id] = "HLT_Ele115_CaloIdVT_"; 
      }
      else if ( (*trig_name)[i].find("HLT_Mu50")!= string::npos      ) { 
        Mu50_trig_id = i; trg_prefix[Mu50_trig_id] = "HLT_Mu50_"; 
      }
      else if ( (*trig_name)[i].find("HLT_TkMu50")!= string::npos      ) { 
        TkMu50_trig_id = i;
      }
    }
    ...
    if (channel == "ee") {
      if (lep0flavor == 'e' && lep1flavor == 'e') {
        ...
        //Reference Trigger: HLT_Ele115_CaloIdVT_ trigger
        if ( !(*trig_passed)[Ele115_trig_id] ) continue;
        ...
        if ( ele_pt[0] < 120 || fabs(ele_eta[0]) > 2.5 ) continue;
        ...
        FillHist2D(Form(trg_prefix[Ele115_trig_id]+"%s_eta_pT_BS",probelep.data()), fabs(ele_etaSupClust[1]), ele_pt[1]>110. ? 150. : ele_pt[1], 1);
        if ( (*trig_passed)[DoubleEle_trig_id] )
          FillHist2D(Form(trg_prefix[DoubleEle_trig_id]+"%s_eta_pT_BS",probelep.data()), 
	                  fabs(ele_etaSupClust[1]), ele_pt[1]>110. ? 150. : ele_pt[1], 1);
        if (fabs(ele_etaSupClust[1]) < 2.5) {
          FillHist1D(Form(trg_prefix[Ele115_trig_id]+"%s_pT_BS",probelep.data()), ele_pt[1], 1);
          if ( (*trig_passed)[DoubleEle_trig_id] ) 
            FillHist1D(Form(trg_prefix[DoubleEle_trig_id]+"%s_pT_BS",probelep.data()), ele_pt[1], 1);
        }
        if (ele_pt[1] > 36) {
          FillHist1D(Form(trg_prefix[Ele115_trig_id]+"%s_eta_BS",probelep.data()), ele_etaSupClust[1], 1);
          if ( (*trig_passed)[DoubleEle_trig_id] )
            FillHist1D(Form(trg_prefix[DoubleEle_trig_id]+"%s_eta_BS",probelep.data()), ele_etaSupClust[1], 1);
        }
      ...
      if (lep0flavor != lep1flavor) {
        ...
        FillHist2D(Form(trg_prefix[Ele115_trig_id]+"%s_eta_pT_BS",probelep.data()), fabs(muon_eta[0]), muon_pt[0]>400 ? 600. : muon_pt[0] , 1);
        if ( (*trig_passed)[Mu50_trig_id] || (TkMu50_trig_id!= -1 &&(*trig_passed)[TkMu50_trig_id]) )
          FillHist2D(Form(trg_prefix[Mu50_trig_id]+"%s_eta_pT_BS",probelep.data()), fabs(muon_eta[0]), muon_pt[0]>400 ? 600. : muon_pt[0] , 1); 
      
        if (fabs(muon_eta[0]) < 2.4) {
          FillHist1D(Form(trg_prefix[Ele115_trig_id]+"%s_pT_BS",probelep.data()), muon_pt[0], 1);
          if ( (*trig_passed)[Mu50_trig_id] || (TkMu50_trig_id!= -1 &&(*trig_passed)[TkMu50_trig_id]) ) 
	    FillHist1D(Form(trg_prefix[Mu50_trig_id]+"%s_pT_BS",probelep.data()), muon_pt[0], 1);
        }
        if (muon_pt[0] > 53) {
          FillHist1D(Form(trg_prefix[Ele115_trig_id]+"%s_eta_BS",probelep.data()), muon_eta[0], 1);
          if ( (*trig_passed)[Mu50_trig_id] || (TkMu50_trig_id!= -1 &&(*trig_passed)[TkMu50_trig_id]) ) 
            FillHist1D(Form(trg_prefix[Mu50_trig_id]+"%s_eta_BS",probelep.data()), muon_eta[0], 1);
        }
    ...
    TLorentzVector electron_sub, muon_sub;
    electron_sub.SetPtEtaPhiM(ele_pt[1], ele_eta[1], ele_phi[1], ELEMASS); // in ee channel for trigger subele leg
    muon_sub.SetPtEtaPhiM(muon_pt[0], muon_eta[0], muon_phi[0], MUONMASS); // in em channel for trigger mu leg

    vector<pair<int, double> > jet_index_corrpt;
    TLorentzVector minjet0, minjet1;
    double rmin0=99, rmin1=99, rmin_ele_sub=99, rmin_muon_sub=99;
    ...
    for (int i=0; i<nJet; i++) {
      ...
      if (jet.Pt()>15 && fabs(jet_eta[i])<3.) {
        ...
        if (testLeg=="eleLeg"){ if (electron_sub.DeltaR(jet) < rmin_ele_sub) rmin_ele_sub = electron_sub.DeltaR(jet);}
        if (testLeg=="muLeg") { if (muon_sub.DeltaR(jet) < rmin_muon_sub) rmin_muon_sub = muon_sub.DeltaR(jet);}
    ...
    if (testLeg=="eleLeg"){
      ...
      if (fabs(ele_etaSupClust[1]) < 2.5) {
        if (ele_pt[1] > 36 ){
          FillHist1D(Form(trg_prefix[Ele115_trig_id]+"%s_rmin_BS",probelep.data()), rmin_ele_sub, 1);
          if ( (*trig_passed)[DoubleEle_trig_id] )
            FillHist1D(Form(trg_prefix[DoubleEle_trig_id]+"%s_rmin_BS",probelep.data()), rmin_ele_sub, 1);
        } 
      }

      if (rmin1<0.4) {
        FillHist2D(Form("Onbt_"+trg_prefix[Ele115_trig_id]+"%s_eta_pT_BS",probelep.data()), 
                   fabs(ele_etaSupClust[1]), ele_pt[1]>110. ? 150. : ele_pt[1], 1);
        if ( (*trig_passed)[DoubleEle_trig_id] )
          FillHist2D(Form("Onbt_"+trg_prefix[DoubleEle_trig_id]+"%s_eta_pT_BS",probelep.data()), 
		     fabs(ele_etaSupClust[1]), ele_pt[1]>110. ? 150. : ele_pt[1], 1);
        if (fabs(ele_etaSupClust[1]) < 2.5) {
          FillHist1D(Form("Onbt_"+trg_prefix[Ele115_trig_id]+"%s_pT_BS",probelep.data()), ele_pt[1], 1);
          if ( (*trig_passed)[DoubleEle_trig_id] )
            FillHist1D(Form("Onbt_"+trg_prefix[DoubleEle_trig_id]+"%s_pT_BS",probelep.data()), ele_pt[1], 1);
        }
        if (ele_pt[1] > 36) {
          FillHist1D(Form("Onbt_"+trg_prefix[Ele115_trig_id]+"%s_eta_BS",probelep.data()), ele_etaSupClust[1], 1);
          if ( (*trig_passed)[DoubleEle_trig_id] )
            FillHist1D(Form("Onbt_"+trg_prefix[DoubleEle_trig_id]+"%s_eta_BS",probelep.data()), ele_etaSupClust[1], 1);
        }
        if (fabs(ele_etaSupClust[1]) < 2.5) {
          if (ele_pt[1] > 36 ){
            FillHist1D(Form("Onbt_"+trg_prefix[Ele115_trig_id]+"%s_rmin_BS",probelep.data()), rmin_ele_sub, 1);
            if ( (*trig_passed)[DoubleEle_trig_id] )
              FillHist1D(Form("Onbt_"+trg_prefix[DoubleEle_trig_id]+"%s_rmin_BS",probelep.data()), rmin_ele_sub, 1);
          } 
        }
      }

      if (rmin1>0.4) {
        FillHist2D(Form("Onnb_"+trg_prefix[Ele115_trig_id]+"%s_eta_pT_BS",probelep.data()),
                        fabs(ele_etaSupClust[1]), ele_pt[1]>110. ? 150. : ele_pt[1], 1);
        if ( (*trig_passed)[DoubleEle_trig_id] )
          FillHist2D(Form("Onnb_"+trg_prefix[DoubleEle_trig_id]+"%s_eta_pT_BS",probelep.data()), 
		     fabs(ele_etaSupClust[1]), ele_pt[1]>110. ? 150. : ele_pt[1], 1);
        if (fabs(ele_etaSupClust[1]) < 2.5) {
          FillHist1D(Form("Onnb_"+trg_prefix[Ele115_trig_id]+"%s_pT_BS",probelep.data()), ele_pt[1], 1);
          if ( (*trig_passed)[DoubleEle_trig_id] )
            FillHist1D(Form("Onnb_"+trg_prefix[DoubleEle_trig_id]+"%s_pT_BS",probelep.data()), ele_pt[1], 1);
        }
        if (ele_pt[1] > 36) {
          FillHist1D(Form("Onnb_"+trg_prefix[Ele115_trig_id]+"%s_eta_BS",probelep.data()), ele_etaSupClust[1], 1);
          if ( (*trig_passed)[DoubleEle_trig_id] )
            FillHist1D(Form("Onnb_"+trg_prefix[DoubleEle_trig_id]+"%s_eta_BS",probelep.data()), ele_etaSupClust[1], 1);
        }
        if (fabs(ele_etaSupClust[1]) < 2.5) {
          if (ele_pt[1] > 36 ){
            FillHist1D(Form("Onnb_"+trg_prefix[Ele115_trig_id]+"%s_rmin_BS",probelep.data()), rmin_ele_sub, 1);
            if ( (*trig_passed)[DoubleEle_trig_id] ) 
              FillHist1D(Form("Onnb_"+trg_prefix[DoubleEle_trig_id]+"%s_rmin_BS",probelep.data()), rmin_ele_sub, 1);
          } 
        }
      }
    ...
    if (testLeg=="muLeg"){
      ...
      if (fabs(muon_eta[0]) < 2.4) {
        if (muon_pt[0] > 53 ){
          FillHist1D(Form(trg_prefix[Ele115_trig_id]+"%s_rmin_BS",probelep.data()), rmin_muon_sub, 1);
          if ( (*trig_passed)[Mu50_trig_id] || (TkMu50_trig_id!= -1 &&(*trig_passed)[TkMu50_trig_id]) )
            FillHist1D(Form(trg_prefix[Mu50_trig_id]+"%s_rmin_BS",probelep.data()), rmin_muon_sub, 1);
        } 
      }

      if ( (lep0flavor == 'm' && rmin0<0.4) || (lep1flavor == 'm' && rmin1<0.4) ) {
        FillHist2D(Form("Onbt_"+trg_prefix[Ele115_trig_id]+"%s_eta_pT_BS",probelep.data()), fabs(muon_eta[0]), muon_pt[0]>400 ? 600. : muon_pt[0] , 1);
        if ( (*trig_passed)[Mu50_trig_id] || (TkMu50_trig_id!= -1 &&(*trig_passed)[TkMu50_trig_id]) )
	  FillHist2D(Form("Onbt_"+trg_prefix[Mu50_trig_id]+"%s_eta_pT_BS",probelep.data()), fabs(muon_eta[0]), muon_pt[0]>400 ? 600. : muon_pt[0] , 1);
        
        if (fabs(muon_eta[0]) < 2.4) {
          FillHist1D(Form("Onbt_"+trg_prefix[Ele115_trig_id]+"%s_pT_BS",probelep.data()), muon_pt[0], 1);
          if ( (*trig_passed)[Mu50_trig_id] || (TkMu50_trig_id!= -1 &&(*trig_passed)[TkMu50_trig_id]) )
	    FillHist1D(Form("Onbt_"+trg_prefix[Mu50_trig_id]+"%s_pT_BS",probelep.data()), muon_pt[0], 1);
        }
        if (muon_pt[0] > 53) {
          FillHist1D(Form("Onbt_"+trg_prefix[Ele115_trig_id]+"%s_eta_BS",probelep.data()), muon_eta[0], 1);
          if ( (*trig_passed)[Mu50_trig_id] || (TkMu50_trig_id!= -1 &&(*trig_passed)[TkMu50_trig_id]) )
	    FillHist1D(Form("Onbt_"+trg_prefix[Mu50_trig_id]+"%s_eta_BS",probelep.data()), muon_eta[0], 1);
        }
        if (fabs(muon_eta[0]) < 2.4) {
          if (muon_pt[0] > 53 ){
            FillHist1D(Form("Onbt_"+trg_prefix[Ele115_trig_id]+"%s_rmin_BS",probelep.data()), rmin_muon_sub, 1);
            if ( (*trig_passed)[Mu50_trig_id] || (TkMu50_trig_id!= -1 &&(*trig_passed)[TkMu50_trig_id]) ) 
	      FillHist1D(Form("Onbt_"+trg_prefix[Mu50_trig_id]+"%s_rmin_BS",probelep.data()), rmin_muon_sub, 1);
          } 
        }
      }
      ...
      if ( (lep0flavor == 'm' && rmin0>0.4) || (lep1flavor == 'm' && rmin1>0.4) ) {
        ...
        FillHist2D(Form("Onnb_"+trg_prefix[Ele115_trig_id]+"%s_eta_pT_BS",probelep.data()), fabs(muon_eta[0]), muon_pt[0]>400 ? 600. : muon_pt[0] , 1);
        if ( (*trig_passed)[Mu50_trig_id] || (TkMu50_trig_id!= -1 &&(*trig_passed)[TkMu50_trig_id]) )
	  FillHist2D(Form("Onnb_"+trg_prefix[Mu50_trig_id]+"%s_eta_pT_BS",probelep.data()), fabs(muon_eta[0]), muon_pt[0]>400 ? 600. : muon_pt[0] , 1);
        if (fabs(muon_eta[0]) < 2.4) {
          FillHist1D(Form("Onnb_"+trg_prefix[Ele115_trig_id]+"%s_pT_BS",probelep.data()), muon_pt[0], 1);
          if ( (*trig_passed)[Mu50_trig_id] || (TkMu50_trig_id!= -1 &&(*trig_passed)[TkMu50_trig_id]) )
	    FillHist1D(Form("Onnb_"+trg_prefix[Mu50_trig_id]+"%s_pT_BS",probelep.data()), muon_pt[0], 1);
        }
        if (muon_pt[0] > 53) {
          FillHist1D(Form("Onnb_"+trg_prefix[Ele115_trig_id]+"%s_eta_BS",probelep.data()), muon_eta[0], 1);
          if ( (*trig_passed)[Mu50_trig_id] || (TkMu50_trig_id!= -1 &&(*trig_passed)[TkMu50_trig_id]) ) 
	    FillHist1D(Form("Onnb_"+trg_prefix[Mu50_trig_id]+"%s_eta_BS",probelep.data()), muon_eta[0], 1);
        }
        if (fabs(muon_eta[0]) < 2.4) {
          if (muon_pt[0] > 53 ){
            FillHist1D(Form("Onnb_"+trg_prefix[Ele115_trig_id]+"%s_rmin_BS",probelep.data()), rmin_muon_sub, 1);
            if ( (*trig_passed)[Mu50_trig_id] || (TkMu50_trig_id!= -1 &&(*trig_passed)[TkMu50_trig_id]) )
	      FillHist1D(Form("Onnb_"+trg_prefix[Mu50_trig_id]+"%s_rmin_BS",probelep.data()), rmin_muon_sub, 1);
          } 
        }
      }
    ...
    if (prefix==7){
      if (testLeg=="eleLeg"){
        FillHist2D(Form(trg_prefix[Ele115_trig_id]+"%s_eta_pT_AS",probelep.data()),
                   fabs(ele_etaSupClust[1]), ele_pt[1]>110. ? 150. : ele_pt[1], 1);
        if ( (*trig_passed)[DoubleEle_trig_id] )  FillHist2D(Form(trg_prefix[DoubleEle_trig_id]+"%s_eta_pT_AS",probelep.data()),
                                                               fabs(ele_etaSupClust[1]), ele_pt[1]>110. ? 150. : ele_pt[1], 1);
        if (fabs(ele_etaSupClust[1]) < 2.5) {
          FillHist1D(Form(trg_prefix[Ele115_trig_id]+"%s_pT_AS",probelep.data()), ele_pt[1], 1);
          if ( (*trig_passed)[DoubleEle_trig_id] ) FillHist1D(Form(trg_prefix[DoubleEle_trig_id]+"%s_pT_AS",probelep.data()), ele_pt[1], 1);
        }
        if (ele_pt[1] > 36) {
          FillHist1D(Form(trg_prefix[Ele115_trig_id]+"%s_eta_AS",probelep.data()), ele_etaSupClust[1], 1);
          if ( (*trig_passed)[DoubleEle_trig_id] ) FillHist1D(Form(trg_prefix[DoubleEle_trig_id]+"%s_eta_AS",probelep.data()), ele_etaSupClust[1], 1);
        }
        if (fabs(ele_etaSupClust[1]) < 2.5) {
          if (ele_pt[1] > 36){
            FillHist1D(Form(trg_prefix[Ele115_trig_id]+"%s_rmin_AS",probelep.data()), rmin_ele_sub, 1);
            if ( (*trig_passed)[DoubleEle_trig_id] ) FillHist1D(Form(trg_prefix[DoubleEle_trig_id]+"%s_rmin_AS",probelep.data()), rmin_ele_sub, 1);
          }
        }

        if (rmin1<0.4){

          FillHist2D(Form("Onbt_"+trg_prefix[Ele115_trig_id]+"%s_eta_pT_AS",probelep.data()),
	                  fabs(ele_etaSupClust[1]), ele_pt[1]>110. ? 150. : ele_pt[1], 1);
          if ( (*trig_passed)[DoubleEle_trig_id] )  FillHist2D(Form("Onbt_"+trg_prefix[DoubleEle_trig_id]+"%s_eta_pT_AS",probelep.data()),
                                                               fabs(ele_etaSupClust[1]), ele_pt[1]>110. ? 150. : ele_pt[1], 1);
          if (fabs(ele_etaSupClust[1]) < 2.5) {
            FillHist1D(Form("Onbt_"+trg_prefix[Ele115_trig_id]+"%s_pT_AS",probelep.data()), ele_pt[1], 1);
            if ( (*trig_passed)[DoubleEle_trig_id] ) FillHist1D(Form("Onbt_"+trg_prefix[DoubleEle_trig_id]+"%s_pT_AS",probelep.data()), ele_pt[1], 1);
          }
          if (ele_pt[1] > 36) {
            FillHist1D(Form("Onbt_"+trg_prefix[Ele115_trig_id]+"%s_eta_AS",probelep.data()), ele_etaSupClust[1], 1);
            if ( (*trig_passed)[DoubleEle_trig_id] ) FillHist1D(Form("Onbt_"+trg_prefix[DoubleEle_trig_id]+"%s_eta_AS",probelep.data()),
							              ele_etaSupClust[1], 1);
          }
          if (fabs(ele_etaSupClust[1]) < 2.5) {
            if (ele_pt[1] > 36){
              FillHist1D(Form("Onbt_"+trg_prefix[Ele115_trig_id]+"%s_rmin_AS",probelep.data()), rmin_ele_sub, 1);
              if ( (*trig_passed)[DoubleEle_trig_id] ) FillHist1D(Form("Onbt_"+trg_prefix[DoubleEle_trig_id]+"%s_rmin_AS",probelep.data()),
								        rmin_ele_sub, 1);
            }
          }
        }
        ...
        if (rmin1>0.4){
          ...
          FillHist2D(Form("Onnb_"+trg_prefix[Ele115_trig_id]+"%s_eta_pT_AS",probelep.data()), 
                     fabs(ele_etaSupClust[1]), ele_pt[1]>110. ? 150. : ele_pt[1], 1);
          if ( (*trig_passed)[DoubleEle_trig_id] )  FillHist2D(Form("Onnb_"+trg_prefix[DoubleEle_trig_id]+"%s_eta_pT_AS",probelep.data()),
                                                               fabs(ele_etaSupClust[1]), ele_pt[1]>110. ? 150. : ele_pt[1], 1);
          if (fabs(ele_etaSupClust[1]) < 2.5) {
            FillHist1D(Form("Onnb_"+trg_prefix[Ele115_trig_id]+"%s_pT_AS",probelep.data()), ele_pt[1], 1);
            if ( (*trig_passed)[DoubleEle_trig_id] ) FillHist1D(Form("Onnb_"+trg_prefix[DoubleEle_trig_id]+"%s_pT_AS",probelep.data()), ele_pt[1], 1);
          }
          if (ele_pt[1] > 36) {
            FillHist1D(Form("Onnb_"+trg_prefix[Ele115_trig_id]+"%s_eta_AS",probelep.data()), ele_etaSupClust[1], 1);
            if ( (*trig_passed)[DoubleEle_trig_id] ) FillHist1D(Form("Onnb_"+trg_prefix[DoubleEle_trig_id]+"%s_eta_AS",probelep.data()),
								      ele_etaSupClust[1], 1);
          }
          if (fabs(ele_etaSupClust[1]) < 2.5) {
            if (ele_pt[1] > 36){
              FillHist1D(Form("Onnb_"+trg_prefix[Ele115_trig_id]+"%s_rmin_AS",probelep.data()), rmin_ele_sub, 1);
              if ( (*trig_passed)[DoubleEle_trig_id] ) FillHist1D(Form("Onnb_"+trg_prefix[DoubleEle_trig_id]+"%s_rmin_AS",probelep.data()),
							               rmin_ele_sub, 1);
            }
          }
        }
      ...
      if (testLeg=="muLeg"){
        ...
        FillHist2D(Form(trg_prefix[Ele115_trig_id]+"%s_eta_pT_AS",probelep.data()), fabs(muon_eta[0]), muon_pt[0]>400 ? 600. : muon_pt[0] , 1);
        if ( (*trig_passed)[Mu50_trig_id] || (TkMu50_trig_id!= -1 &&(*trig_passed)[TkMu50_trig_id]) )  
          FillHist2D(Form(trg_prefix[Mu50_trig_id]+"%s_eta_pT_AS",probelep.data()),fabs(muon_eta[0]), muon_pt[0]>400 ? 600. : muon_pt[0] , 1);
        if (fabs(muon_eta[0]) < 2.4) {
          FillHist1D(Form(trg_prefix[Ele115_trig_id]+"%s_pT_AS",probelep.data()), muon_pt[0], 1);
          if ( (*trig_passed)[Mu50_trig_id] || (TkMu50_trig_id!= -1 &&(*trig_passed)[TkMu50_trig_id]) ) 
            FillHist1D(Form(trg_prefix[Mu50_trig_id]+"%s_pT_AS",probelep.data()), muon_pt[0], 1);
        }
        if (muon_pt[0] > 53) {
          FillHist1D(Form(trg_prefix[Ele115_trig_id]+"%s_eta_AS",probelep.data()), muon_eta[0], 1);
          if ( (*trig_passed)[Mu50_trig_id] || (TkMu50_trig_id!= -1 &&(*trig_passed)[TkMu50_trig_id]) ) 
            FillHist1D(Form(trg_prefix[Mu50_trig_id]+"%s_eta_AS",probelep.data()), muon_eta[0], 1);
        }
        if (fabs(muon_eta[0]) < 2.4) {
          if (muon_pt[0] > 53){
            FillHist1D(Form(trg_prefix[Ele115_trig_id]+"%s_rmin_AS",probelep.data()), rmin_muon_sub, 1);
            if ( (*trig_passed)[Mu50_trig_id] || (TkMu50_trig_id!= -1 &&(*trig_passed)[TkMu50_trig_id]) ) 
              FillHist1D(Form(trg_prefix[Mu50_trig_id]+"%s_rmin_AS",probelep.data()), rmin_muon_sub, 1);
          }
        }

        if ( (lep0flavor == 'm' && rmin0<0.4) || (lep1flavor == 'm' && rmin1<0.4) ){

          FillHist2D(Form("Onbt_"+trg_prefix[Ele115_trig_id]+"%s_eta_pT_AS",probelep.data()), fabs(muon_eta[0]), muon_pt[0]>400 ? 600. : muon_pt[0] , 1);
          if ( (*trig_passed)[Mu50_trig_id] || (TkMu50_trig_id!= -1 &&(*trig_passed)[TkMu50_trig_id]) )
	    FillHist2D(Form("Onbt_"+trg_prefix[Mu50_trig_id]+"%s_eta_pT_AS",probelep.data()), fabs(muon_eta[0]), muon_pt[0]>400 ? 600. : muon_pt[0] , 1);
          if (fabs(muon_eta[0]) < 2.4) {
            FillHist1D(Form("Onbt_"+trg_prefix[Ele115_trig_id]+"%s_pT_AS",probelep.data()), muon_pt[0], 1);
            if ((*trig_passed)[Mu50_trig_id] || (TkMu50_trig_id!= -1 &&(*trig_passed)[TkMu50_trig_id]) ) 
              FillHist1D(Form("Onbt_"+trg_prefix[Mu50_trig_id]+"%s_pT_AS",probelep.data()), muon_pt[0], 1);
          }
          if (muon_pt[0] > 53) {
            FillHist1D(Form("Onbt_"+trg_prefix[Ele115_trig_id]+"%s_eta_AS",probelep.data()), muon_eta[0], 1);
            if ( (*trig_passed)[Mu50_trig_id] || (TkMu50_trig_id!= -1 &&(*trig_passed)[TkMu50_trig_id])) 
              FillHist1D(Form("Onbt_"+trg_prefix[Mu50_trig_id]+"%s_eta_AS",probelep.data()), muon_eta[0], 1);
          }
          if (fabs(muon_eta[0]) < 2.4) {
            if (muon_pt[0] > 53){
              FillHist1D(Form("Onbt_"+trg_prefix[Ele115_trig_id]+"%s_rmin_AS",probelep.data()), rmin_muon_sub, 1);
              if ( (*trig_passed)[Mu50_trig_id] || (TkMu50_trig_id!= -1 &&(*trig_passed)[TkMu50_trig_id]) ) 
                FillHist1D(Form("Onbt_"+trg_prefix[Mu50_trig_id]+"%s_rmin_AS",probelep.data()), rmin_muon_sub, 1);
            }
          }
        }
        ...
        if ((lep0flavor == 'm' && rmin0>0.4) || (lep1flavor == 'm' && rmin1>0.4)){
          ...
          FillHist2D(Form("Onnb_"+trg_prefix[Ele115_trig_id]+"%s_eta_pT_AS",probelep.data()), fabs(muon_eta[0]), muon_pt[0]>400 ? 600. : muon_pt[0] , 1);
          if ( (*trig_passed)[Mu50_trig_id] || (TkMu50_trig_id!= -1 &&(*trig_passed)[TkMu50_trig_id]) )  
	    FillHist2D(Form("Onnb_"+trg_prefix[Mu50_trig_id]+"%s_eta_pT_AS",probelep.data()), fabs(muon_eta[0]), muon_pt[0]>400 ? 600. : muon_pt[0] , 1);
          if (fabs(muon_eta[0]) < 2.4) {
            FillHist1D(Form("Onnb_"+trg_prefix[Ele115_trig_id]+"%s_pT_AS",probelep.data()), muon_pt[0], 1);
            if ( (*trig_passed)[Mu50_trig_id] || (TkMu50_trig_id!= -1 &&(*trig_passed)[TkMu50_trig_id]) ) 
              FillHist1D(Form("Onnb_"+trg_prefix[Mu50_trig_id]+"%s_pT_AS",probelep.data()), muon_pt[0], 1);
          }
          if (muon_pt[0] > 53) {
            FillHist1D(Form("Onnb_"+trg_prefix[Ele115_trig_id]+"%s_eta_AS",probelep.data()), muon_eta[0], 1);
            if ( (*trig_passed)[Mu50_trig_id] || (TkMu50_trig_id!= -1 &&(*trig_passed)[TkMu50_trig_id]) ) 
              FillHist1D(Form("Onnb_"+trg_prefix[Mu50_trig_id]+"%s_eta_AS",probelep.data()), muon_eta[0], 1);
          }
          if (fabs(muon_eta[0]) < 2.4) {
            if (muon_pt[0] > 53){
              FillHist1D(Form("Onnb_"+trg_prefix[Ele115_trig_id]+"%s_rmin_AS",probelep.data()), rmin_muon_sub, 1);
              if ( (*trig_passed)[Mu50_trig_id] || (TkMu50_trig_id!= -1 &&(*trig_passed)[TkMu50_trig_id]) )
                FillHist1D(Form("Onnb_"+trg_prefix[Mu50_trig_id]+"%s_rmin_AS",probelep.data()), rmin_muon_sub, 1);
            }
          }
        }
  ...
  if ( testLeg=="eleLeg" ) {
   outFile->mkdir("HLT_Ele115_CaloIdVT_"); outFile->mkdir("HLT_DoubleEle33_CaloIdL_");
   outFile->mkdir("Onnb_HLT_Ele115_CaloIdVT_"); outFile->mkdir("Onnb_HLT_DoubleEle33_CaloIdL_");
   outFile->mkdir("Onbt_HLT_Ele115_CaloIdVT_"); outFile->mkdir("Onbt_HLT_DoubleEle33_CaloIdL_");
  }
  else if ( testLeg=="muLeg" ) {
   outFile->mkdir("HLT_Ele115_CaloIdVT_"); outFile->mkdir("HLT_Mu50_");
   outFile->mkdir("Onnb_HLT_Ele115_CaloIdVT_"); outFile->mkdir("Onnb_HLT_Mu50_");
   outFile->mkdir("Onbt_HLT_Ele115_CaloIdVT_"); outFile->mkdir("Onbt_HLT_Mu50_");
  }

  for (map<TString, TH1*>::iterator hid = m_Histos1D.begin(); hid != m_Histos1D.end(); hid++) {
    outFile->cd();
    TString prefix = hid->first(0, 1);
    for (map<int, TString>::iterator trig = trg_prefix.begin(); trig != trg_prefix.end(); trig++) {
      if ( hid->first.Contains("Onnb_"+trig->second) ) outFile->cd(outName + ":/Onnb_" + trig->second ); 
      else if ( hid->first.Contains("Onbt_"+trig->second) ) outFile->cd(outName + ":/Onbt_" + trig->second ); 
      else if ( hid->first.Contains(trig->second) ) outFile->cd(outName + ":/" + trig->second ); 
    }
...
void setPars(const string& parFile) {
  ...
  while (getline(file, line)){
    ...
    else if (var == "testLeg") testLeg = line;
    else if (var == "tagtrig") tagtrig = line;
    else if (var == "probetrig") probetrig = line;
